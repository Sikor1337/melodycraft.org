...
- `src/types.ts` — shared types: `Tier` (`'standard' | 'premium' | 'signature'`), `SongOrder` (the order-form brief), `OrderItem` (what checkout receives), and `TIER_PRICE` (`standard` = $9.99, `premium` = $39.99, `signature` = $69.99), plus `TIER_LABEL` / `TIER_NOTE`.
